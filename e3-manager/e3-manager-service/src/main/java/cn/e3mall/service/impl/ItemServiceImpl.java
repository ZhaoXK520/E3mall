...
public class ItemServiceImpl implements ItemService {
	...
	public E3Result updateItem(TbItem item, String desc) {
		// TODO Auto-generated method stub
		TbItem itemOld = itemMapper.selectByPrimaryKey(item.getId());
		item.setStatus(itemOld.getStatus());
		item.setCreated(itemOld.getCreated());
		item.setUpdated(new Date());
	...
	@Override
	public E3Result deleteItemById(List itemId) {
		TbItemExample itemExample = new TbItemExample();
		Criteria itemcriteria = itemExample.createCriteria();
		itemcriteria.andIdIn(itemId);
		itemMapper.deleteByExample(itemExample);
		TbItemDescExample descexample = new TbItemDescExample();
		cn.e3mall.pojo.TbItemDescExample.Criteria desccriteria = descexample.createCriteria();
		desccriteria.andItemIdIn(itemId);
		itemDescMapper.deleteByExample(descexample);
		return E3Result.ok();
	}

	@Override
	public E3Result instockItemById(List ids) {
		TbItemExample example = new TbItemExample();
	...
	@Override
	public E3Result reshelfItemById(List ids) {
		TbItemExample example = new TbItemExample();
		Criteria itemcriteria = example.createCriteria();
		itemcriteria.andIdIn(ids);
		TbItem item = new TbItem();
